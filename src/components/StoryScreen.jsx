...
export default function StoryScreen({ levelId, onNavigate }) {
  ...
  return (
    <div className="scanlines min-h-screen war-room-bg flex flex-col" onClick={skipTyping}>
      {/* Corner decorations */}
      <div className="fixed top-0 left-0 w-12 h-12 border-l-2 border-t-2 border-red-900/30 pointer-events-none" />
      <div className="fixed top-0 right-0 w-12 h-12 border-r-2 border-t-2 border-red-900/30 pointer-events-none" />
      <div className="fixed bottom-0 left-0 w-12 h-12 border-l-2 border-b-2 border-red-900/30 pointer-events-none" />
      <div className="fixed bottom-0 right-0 w-12 h-12 border-r-2 border-b-2 border-red-900/30 pointer-events-none" />

      {/* Top bar */}
      <div className="relative z-10 border-b border-red-900/30 bg-black/80">
        <div className="flex items-center justify-between px-5 py-2.5 max-w-4xl mx-auto w-full">
          <button
            onClick={(e) => { e.stopPropagation(); handleBack(); }}
            className="pixel-text text-red-800 text-[10px] hover:text-red-400 cursor-pointer"
          >
            BACK
          </button>
          <div className="flex items-center gap-4">
            ...
            <div className="pixel-text text-red-500 text-[10px]">
        ...
        <div className="w-full max-w-3xl">
          ...
          {showIntro && (
            <div className="fade-in-up">
              ...
              <div className="text-center mb-10">
                <h1 className="pixel-text text-xl md:text-3xl text-red-500 glow-red mb-4 leading-relaxed">
                  {story.title}
                </h1>
                <div className="flex items-center justify-center gap-4">
                  <div className="h-px w-12 bg-red-800" />
                  <span className="pixel-text text-[10px] text-red-800">{story.year}</span>
                  <div className="h-px w-12 bg-red-800" />
                </div>
              </div>
              ...
              <div className="border border-red-900/30 bg-black/40 p-6 md:p-8 mb-8">
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <span className="pixel-text text-[8px] text-amber-700">INCOMING TRANSMISSION</span>
                </div>
              ...
              {showChoices && (
                <div className="fade-in" onClick={(e) => e.stopPropagation()}>
                  <button
                    ...
                    className="pixel-btn w-full bg-red-950/60 border-red-700 text-red-100 px-8 py-5 text-[10px] hover:bg-red-900/80 hover:border-red-400 flex items-center justify-center gap-3"
                  >
                    ACCEPT MISSION
                  </button>
                </div>
          ...
          {!showIntro && (
            <div className="fade-in flex flex-col">
              ...
              {currentNode === 'start' && (
                <div className="mb-5">
                  <h2 className="pixel-text text-sm md:text-lg text-red-500 glow-red mb-1">
                    {story.title}
                  </h2>
              ...
              <div
                ref={textRef}
                className="border border-red-900/20 bg-black/50 p-6 md:p-8 overflow-y-auto mb-6"
                ...
                {showChoices && storyParts.length > 0 ? (
                  <div className="space-y-5">
                    {storyParts.map((part, i) => {
                      if (part.type === 'fact') {
                        return (
                          <div key={i} className="border-l-2 border-amber-700/60 bg-amber-950/20 px-4 py-3">
                            <div className="pixel-text text-[7px] text-amber-700 mb-2 flex items-center gap-2">
                              HISTORICAL RECORD
                            </div>
                      ...
                      if (part.type === 'lessons') {
                        return (
                          <div key={i} className="border border-green-900/40 bg-green-950/10 p-4">
                            <div className="pixel-text text-[7px] text-green-600 mb-3 flex items-center gap-2">
                              KEY INTELLIGENCE
                            </div>
              ...
              {isTyping && (
                <div className="pixel-text text-[7px] text-gray-700 text-center mb-4">
                  ...
                  {isEnding ? (
                    <div className="flex flex-col sm:flex-row gap-3">
                      <button
                        ...
                        className="pixel-btn flex-1 bg-red-950/40 border-red-800 text-red-200 px-6 py-5 text-[10px] hover:bg-red-900/60 hover:border-red-500"
                      >
                        REPLAY MISSION
                      </button>
                      <button
                        onClick={() => onNavigate('levelselect')}
                        className="pixel-btn flex-1 bg-gray-950/40 border-gray-700 text-gray-400 px-6 py-5 text-[10px] hover:bg-gray-900/60 hover:border-gray-500"
                      >
                        NEW CONFLICT
                      </button>
                    </div>
                    ...
                    <div>
                      <div className="pixel-text text-[8px] text-red-600 mb-4 flex items-center gap-2">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        {node?.choices.map((choice, i) => (
                          <button
                            ...
                            className="group w-full text-left border-2 border-red-900/40 bg-red-950/10 hover:bg-red-900/30 hover:border-red-500 transition-all duration-150 cursor-pointer"
                          >
                            <div className="flex items-start gap-4 p-5">
                              <div className="pixel-text text-sm text-red-500 bg-red-950/60 w-9 h-9 flex items-center justify-center shrink-0 border border-red-800/50 group-hover:bg-red-700/60 group-hover:text-white">
                                {String.fromCharCode(65 + i)}
                              </div>
                              <div className="terminal-text text-lg md:text-xl text-red-200/80 group-hover:text-red-100 leading-snug pt-1">
                                {choice.text}
                              </div>
                            </div>
                          </button>
      ...
      {/* Bottom bar */}
      <div className="relative z-10 border-t border-red-900/20 bg-black/80 px-5 py-2">
        <div className="flex items-center justify-between max-w-4xl mx-auto pixel-text text-[7px] text-red-900/50">
